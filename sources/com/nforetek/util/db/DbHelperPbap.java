...
public class DbHelperPbap extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sQLiteDatabase) {
        ...
        StringBuilder sb2 = new StringBuilder("CREATE TABLE IF NOT EXISTS AddressDetail (");
        ...
        sb2.append(" nvarchar(100),");
        sb2.append("FOREIGN KEY(");
        sb2.append(ADDRESS_DETAIL_FIELD[1]);
        sb2.append(") REFERENCES ");
        sb2.append(PHONEBOOK_CONTENT);
        sb2.append("(");
        sb2.append(PHONEBOOK_CONTENT_FIELD[0]);
        sb2.append(") ");
        sb2.append(");");
        sQLiteDatabase.execSQL(sb2.toString());
        sQLiteDatabase.execSQL("CREATE TABLE IF NOT EXISTS EmailDetail (" + EMAIL_DETAIL_FIELD[0] + " INTEGER primary key autoincrement, " + EMAIL_DETAIL_FIELD[1] + " INTEGER, " + EMAIL_DETAIL_FIELD[2] + " INTEGER, " + EMAIL_DETAIL_FIELD[3] + " nvarchar(100),FOREIGN KEY(" + EMAIL_DETAIL_FIELD[1] + ") REFERENCES " + PHONEBOOK_CONTENT + "(" + PHONEBOOK_CONTENT_FIELD[0] + ") );");
        StringBuilder sb3 = new StringBuilder("CREATE TABLE IF NOT EXISTS PhoneType (");
        sb3.append(PHONE_TYPE_FIELD[0]);
        sb3.append(" nvarchar(5) , ");
        sb3.append(PHONE_TYPE_FIELD[1]);
        sb3.append(" nvarchar(26) );");
        sQLiteDatabase.execSQL(sb3.toString());
        sQLiteDatabase.execSQL("CREATE TABLE IF NOT EXISTS CallHistory (" + CALLHISTORY_FIELD[0] + " INTEGER primary key autoincrement, " + CALLHISTORY_FIELD[1] + " nvarchar(16), " + CALLHISTORY_FIELD[2] + " nvarchar(8), " + CALLHISTORY_FIELD[3] + " nvarchar(8), " + CALLHISTORY_FIELD[4] + " nvarchar(30) not null, " + CALLHISTORY_FIELD[5] + " nvarchar(10) not null, " + CALLHISTORY_FIELD[6] + " nvarchar(5) not null, " + CALLHISTORY_FIELD[7] + " nvarchar(20) not null, " + CALLHISTORY_FIELD[8] + " nvarchar(8) not null, " + CALLHISTORY_FIELD[9] + " nvarchar(6) not null);");
    ...
    public int insertNumberType(SQLiteDatabase sQLiteDatabase, ArrayList<HashMap<String, String>> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            try {
                ...
                contentValues.put("TypeName", hashMap.get("TypeName"));
                j = sQLiteDatabase.insert(PHONE_TYPE, null, contentValues);
            } catch (Exception e) {
                e.printStackTrace();
            ...
            try {
                ...
                for (int i = 0; i < elementsByTagName.getLength(); i++) {
                    ...
                    if (attributes.getNamedItem("Type").getTextContent().trim().length() > 0) {
                        hashMap.put("Type", attributes.getNamedItem("Type").getTextContent());
                    }
                    if (attributes.getNamedItem("TypeName").getTextContent().trim().length() > 0) {
                        hashMap.put("TypeName", attributes.getNamedItem("TypeName").getTextContent());
                    }
                    arrayList.add(hashMap);
                }
                Cursor isEmptyPhoneType = isEmptyPhoneType(sQLiteDatabase);
                if (isEmptyPhoneType != null) {
                    isEmptyPhoneType.moveToNext();
                    if (isEmptyPhoneType.getInt(isEmptyPhoneType.getColumnIndex("amount")) <= 0) {
                        insertNumberType(sQLiteDatabase, arrayList);
                    }
                }
                isEmptyPhoneType.close();
            } catch (Exception e) {
                e.printStackTrace();
                Log.d(this.TAG, e.getMessage());
            }
    ...
    public Cursor queryContacterByLikePhoneNumAndAddress(SQLiteDatabase sQLiteDatabase, String str, String str2, int i) {
        if (i == 0) {
            str = "%" + str + "%";
        } else if (i == 1) {
        ...
        return sQLiteDatabase.rawQuery("select a.FullName, b.Number from PhoneBookcontent a inner join phonenumberdetail b on a._id = b.Content_id where b.Number like ? and a.cellphone_address = ? order by b.Number;", new String[]{str, str2});
    }
    ...
    public int deleteVcardInfo(SQLiteDatabase sQLiteDatabase, String str, int i) {
        int i2;
        Cursor query = sQLiteDatabase.query(PHONEBOOK_CONTENT, new String[]{"_id, PhotoPath"}, "CellPhone_Address=? and StorageType=?", new String[]{str, String.valueOf(i)}, null, null, null);
    ...
    public void insertVcardInfo(SQLiteDatabase sQLiteDatabase, VCardPack vCardPack, int i) {
        ContentValues contentValues = new ContentValues();
        if (vCardPack != null) {
            String fullName = vCardPack.getFullName();
            int i2 = 0;
            try {
                i2 = Integer.valueOf(fullName).intValue();
                String str = this.TAG;
                Log.v(str, "numName : " + i2);
            } catch (NumberFormatException unused) {
            }
            if (i2 < 0) {
                String str2 = this.TAG;
                Log.e(str2, "name is " + i2 + " , automatically set it to \"Private Number\"");
                fullName = "Private Number";
            }
    ...
    public void insertCallHistoryInfo(SQLiteDatabase sQLiteDatabase, VCardPack vCardPack, int i) {
        ...
        if (vCardPack != null) {
            ...
            contentValues.put("FullName", fullName == null ? "" : vCardPack.getFullName());
            contentValues.put("FirstName", vCardPack.getFirstName() == null ? "" : vCardPack.getFirstName());
            contentValues.put("LastName", vCardPack.getLastName() == null ? "" : vCardPack.getLastName());
            contentValues.put("CellPhone_Address", vCardPack.getCellPhone_Address() == null ? "" : vCardPack.getCellPhone_Address());
            contentValues.put("StorageType", Integer.valueOf(i));
            for (PhoneInfo phoneInfo : vCardPack.getPhoneNumbers()) {
                contentValues.put(PHONE_TYPE, phoneInfo.getPhoneType() == null ? "" : phoneInfo.getPhoneType());
                contentValues.put("PhoneNumber", phoneInfo.getPhoneNumber() == null ? "" : phoneInfo.getPhoneNumber());
            }
            contentValues.put("HistoryDate", vCardPack.getHistoryDate() == null ? "" : vCardPack.getHistoryDate());
            contentValues.put("HistoryTime", vCardPack.getHistoryTime() != null ? vCardPack.getHistoryTime() : "");
