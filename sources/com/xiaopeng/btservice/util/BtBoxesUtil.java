...
public class BtBoxesUtil {
    ...
    private ServiceConnection mBtServiceConnection = new ServiceConnection() { // from class: com.xiaopeng.btservice.util.BtBoxesUtil.1
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            BtBoxesUtil.this.nForeService = UiCommand.Stub.asInterface(iBinder);
    ...
    public MAPControl getMAPControl(AbsMAPControlCallback absMAPControlCallback) {
        return new MAPControl(this.mContext, absMAPControlCallback);
    }

    public PhoneControl getPhoneControl(AbsPhoneControlCallback absPhoneControlCallback) {
        return new PhoneControl(this.mContext, absPhoneControlCallback);
    }

    public PBAPControl getPBAPControl(AbsPBAPControlCallback absPBAPControlCallback) {
        return new PBAPControl(this.mContext, absPBAPControlCallback);
    }

    public BluetoothControl getBluetoothControl(AbsBluetoothControlCallback absBluetoothControlCallback) {
        return new BluetoothControl(this.mContext, absBluetoothControlCallback);
    }

    public UsbBluetoothControl getUsbBluetoothControl(AbsUsbBluetoothCallback absUsbBluetoothCallback) {
        return new UsbBluetoothControl(this.mContext, absUsbBluetoothCallback);
    }
    ...
    public void connectBluetooth() {
        ...
        intent.setComponent(new ComponentName(PACKAGE_BLUETOOTH_SERVICE, INTENT_BLUETOOTH_SERVICE));
        try {
            String str = TAG;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void notifyConnected(UiCommand uiCommand) {
        String str = TAG;
        ...
        synchronized (this.mConnectionCallbacks) {
            for (AbstractConnection abstractConnection : this.mConnectionCallbacks) {
                abstractConnection.onConnected(uiCommand);
            }
        }
    }
    ...
    public void notifyDisconnected() {
        synchronized (this.mConnectionCallbacks) {
            ...
            for (AbstractConnection abstractConnection : this.mConnectionCallbacks) {
                abstractConnection.unRegister();
            }
        }
    }

    public void registerCallback(AbsControl absControl) {
        registerCallback(absControl.mConnection);
    }

    private void registerCallback(AbstractConnection abstractConnection) {
        synchronized (this.mConnectionCallbacks) {
            this.mConnectionCallbacks.add(abstractConnection);
        ...
        if (uiCommand != null) {
            abstractConnection.register(uiCommand);
        }
    }

    public void unRegisterCallback(AbsControl absControl) {
        unRegisterCallback(absControl.mConnection);
    }

    private void unRegisterCallback(AbstractConnection abstractConnection) {
        abstractConnection.unRegister();
        String str = TAG;
        Log.d(str, "unRegisterCallback from : " + getPackageName());
        synchronized (this.mConnectionCallbacks) {
            this.mConnectionCallbacks.remove(abstractConnection);
        }
    }
}
