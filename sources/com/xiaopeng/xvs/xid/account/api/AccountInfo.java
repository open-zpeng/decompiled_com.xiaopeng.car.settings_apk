...
public class AccountInfo {
    ...
    public AccountType getAccountType() {
        ...
        int i2 = 1;
        try {
            parseInt = Integer.parseInt(getUserData("user_type", "1"));
        } catch (Exception e) {
            e = e;
        }
        try {
            if (parseInt >= AccountType.values().length) {
                i2 = AccountType.values().length - 1;
                L.e(TAG, "getAccountType:throw new ArrayIndexOutOfBoundsException() index=" + i2);
            } else {
                i2 = parseInt;
            }
        } catch (Exception e2) {
            ...
            AccountType accountType = AccountType.values()[i2];
            L.d(TAG, "getAccountType: accountType=" + accountType);
            return accountType;
        }
        AccountType accountType2 = AccountType.values()[i2];
