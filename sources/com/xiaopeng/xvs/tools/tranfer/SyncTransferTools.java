...
public final class SyncTransferTools {
    ...
    public void initDb(final Application application) {
        ...
        new Thread(new Runnable() { // from class: com.xiaopeng.xvs.tools.tranfer.SyncTransferTools.1
            ...
            public void run() {
                ...
                syncTransferTools.mDataAccessor = new DataAccessor(syncTransferTools.mDataCenter.getSyncDataDao());
                ToolsSPUtils.getInstance().init(application);
                SyncTransferTools syncTransferTools2 = SyncTransferTools.this;
                syncTransferTools2.mUidArray = syncTransferTools2.readTransferUidArray();
            }
        }).start();
    }
    ...
    private void eraseDb(long j) {
        ...
        String str = this.mUidArray;
        if (str.contains(j + ",")) {
            String str2 = this.mUidArray;
            ToolsSPUtils.getInstance().putString(KYE_SYNC_TRANSFER_UID_ARRAY, str2.replace(j + ",", ""));
    ...
    private String readOnceAllSyncDataToUidArray() {
        ...
        if (queryUid != null && !queryUid.isEmpty()) {
            for (int i = 0; i < queryUid.size(); i++) {
                sb.append(queryUid.get(i).longValue());
                sb.append(",");
            }
        }
