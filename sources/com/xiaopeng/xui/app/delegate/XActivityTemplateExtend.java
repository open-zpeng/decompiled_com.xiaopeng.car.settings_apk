...
abstract class XActivityTemplateExtend implements XActivityTemplate, XActivityLifecycle {
    ...
    private static class PanelImpl extends BaseTemplate implements XActivityTemplate.Panel {
        ...
        void initWindowAttributes(XActivityWindowAttributes xActivityWindowAttributes) {
            if (FlavorUtils.isV5()) {
                int dip2px = XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_x_v5);
                XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_y_v5);
                int dip2px2 = XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_width_v5);
                xActivityWindowAttributes.setX(dip2px).setY(180).setWidth(dip2px2).setHeight(XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_height_v5)).setGravity(81);
                return;
            }
            int i = this.mActivity.getResources().getConfiguration().orientation;
            int dip2px3 = XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_x);
            int dip2px4 = XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_y);
            int dip2px5 = XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_width);
            int dip2px6 = XuiUtils.dip2px(this.mActivity, R.dimen.x_compat_app_panel_height);
            if (i == 1) {
                xActivityWindowAttributes.setX(dip2px3).setY(dip2px4).setWidth(dip2px5).setHeight(dip2px6).setGravity(80);
            } else if (i != 2) {
            } else {
                xActivityWindowAttributes.setX(dip2px3).setY(dip2px4).setWidth(dip2px5).setHeight(dip2px6).setGravity(8388627);
            }
        }
        ...
        void initWindowVisible(XActivityWindowVisible xActivityWindowVisible) {
            super.initWindowVisible(xActivityWindowVisible);
            if (FlavorUtils.isV5()) {
                xActivityWindowVisible.setAutoVisibleEnableOnPause(false);
            } else {
                xActivityWindowVisible.setAutoVisibleEnableOnPause(true);
            }
        }
